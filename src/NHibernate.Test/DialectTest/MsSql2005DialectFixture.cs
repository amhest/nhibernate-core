...
namespace NHibernate.Test.DialectTest
{
	...
	public class MsSql2005DialectFixture
	{
		...
		[Test]
		public void GetPagedLimitStringWithSqlComments()
		{
			var d = new MsSql2005Dialect();
			var limitSqlQuery = d.GetLimitString(new SqlString(" /* criteria query */ SELECT p from lcdtm"),  new SqlString("10"), new SqlString("20"));
			Assert.That(limitSqlQuery, Is.Not.Null);
         Assert.That(limitSqlQuery.ToString(), Is.EqualTo(" /* criteria query */ SELECT TOP (20) p FROM " +
                                                          "(SELECT p, ROW_NUMBER() OVER(ORDER BY CURRENT_TIMESTAMP) as __hibernate_sort_row from lcdtm) as query " +
                                                          "WHERE query.__hibernate_sort_row > 10 ORDER BY query.__hibernate_sort_row"));
		}
